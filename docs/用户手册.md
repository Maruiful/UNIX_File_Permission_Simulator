...
[root@/]$ chown /home/file.txt 1 3
修改所有者成功: UID=1, GID=3
```

**参数说明**:
- `uid` - 新的用户ID (必需)
- `gid` - 新的组ID (可选,如果不指定则不修改组)

**注意**: 只有 root 可以修改文件所有者
...
| `groupmod <gid> <uid>` | 添加用户到组 | `groupmod 3 1` |
| `su <uid>` | 切换用户 | `su 1` |
| `whoami` | 显示用户 | `whoami` |
...
| `chown <path> <u> [g]` | 修改所有者 | `chown /a 1 2` |
